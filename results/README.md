...
| `guardrail_checklist.json` | `python3 scripts/guardrail_checklist.py --json results/guardrail_checklist.json` | All 12 D3(b) cases, scripted. 12/12. |
...
| `d2c_measured_exact.json` | `python3 scripts/measure_parallel.py --json ...` | The exact (simulated-replay, not closed-form) per-case token measurement behind `d2c_measurement.md`. |
...
| `d2b_live_v1_gpt4o_mini.json`, `d2b_live_v2_gpt4o_mini.json` | `harness.py --live --descriptor-version ...` | Canonical D2(b) descriptor control arms: same model, current 50-case set, 90 trials, descriptor version changed, return shape held fixed. `v1` uses `scripts/descriptors_v1.py` — see `d2b_descriptor_rewrite.md`. |
...
| `d2c_scripted_measured_current.json` | `python3 scripts/measure_parallel.py --json ...` | Current per-case token measurement — supersedes `d2c_measured_exact.json` above. |
| `decisions_live_gpt4o_mini_sample.jsonl` | copied from a live `harness.py --live --model openai/gpt-4o-mini` run's `decisions.jsonl` | A permanent, committed example of the gated action's own persisted record — same shape D1 documents (`ts`, `evidence`, `autonomy`, `gate`, `turns`, real `cost_usd`), 90 records from the current 50-case set. `decisions.jsonl` itself stays git-ignored (it's a run output, truncated and rewritten by every `harness.py`/`scripts/guardrail_checklist.py` run — see `d1_sample_traces.md`), so this is the stable audit copy: what a real live decision record actually looks like, without a re-run overwriting it. |
...
python3 scripts/measure_parallel.py --json results/d2c_measured_exact.json
python3 scripts/guardrail_checklist.py --json results/guardrail_checklist.json
```
