...
# One restrained publication theme for every report figure.
plt.rcParams.update({
    "font.family": "Arial", "font.size": 9.5,
    "axes.titlesize": 12, "axes.titleweight": "bold",
    "axes.titlecolor": NAVY, "axes.labelsize": 9.5,
    "axes.labelcolor": "#374151", "axes.edgecolor": "#374151",
    "axes.linewidth": 0.7, "xtick.labelsize": 8.5,
    "ytick.labelsize": 8.5, "xtick.color": "#374151",
    "ytick.color": "#374151", "grid.color": "#D6DCE5",
    "grid.linewidth": 0.6, "grid.alpha": 0.65,
...
def turn_token_growth_plot():
    ...
    ax.stackplot(turns, prefix, history,
                 ...
                 colors=["#E5EBF1", LIGHT_BLUE], alpha=0.72)
...
def live_battery_summary_plot():
    ...
    model_order = ["DeepSeek", "Gemini", "GPT-4o", "Qwen", "Llama"]
    panels = [
        ("Trial pass rate",
    ...
    for ax, (title, rows, maximum, formatter, highlight) in zip(axes, panels):
        values_by_model = dict(rows)
        rows = [(model, values_by_model[model]) for model in model_order]
        ax.set_xlim(0, 1.58)
...
def descriptor_v1_v2_live_plot():
    ...
    labels = ["v1 (vague)", "v2 (current)"]
    colors = [SLATE, BLUE]
    metrics = [
        ("case-level pass rate", [v1_live["p"] * 100, v2_live["p"] * 100],
         lambda v: "{:.1f}%".format(v)),
        ("cost per run, USD", [v1_live["l1"], v2_live["l1"]],
         lambda v: "${:.5f}".format(v)),
        ("average latency, seconds/run", [v1_live["latency"], v2_live["latency"]],
         lambda v: "{:.1f}s".format(v)),
    ]
    fig, axes = plt.subplots(1, 3, figsize=(11.5, 4.3))
    for ax, (title, values, formatter) in zip(axes, metrics):
        ...
        for bar, value in zip(bars, values):
            ax.text(bar.get_x() + bar.get_width() / 2, value + max(values) * 0.035,
                    formatter(value), ha="center", va="bottom", color=NAVY,
                    fontweight="bold", fontsize=9)
        for spine in ("top", "right", "left"):
            ax.spines[spine].set_visible(False)
    fig.suptitle("Descriptor v1 vs v2, openai/gpt-4o-mini",
                 fontsize=15, fontweight="bold", color=NAVY, x=0.02,
                 ha="left", y=1.01)
    fig.tight_layout(rect=(0, 0, 1, 0.92), w_pad=2)
    save(fig, "fig_d2b_live_v1_v2.png")
...
def cost_sensitivity_all_models_plot():
    """D6 sensitivity, all five models on one shared horizontal axis: each
    model's own measured p, +-10 percentage points, as a floating range
    bar from its upside cost to its downside cost, with the measured point
    marked inside the bar. One chart, one log-scaled x-axis (the ~3000x
    spread between the cheapest upside and the most expensive downside
    would flatten a linear axis), horizontal so model names read normally
    and every range compares directly top-to-bottom - the same visual
    language as monthly_cost_by_model_plot() (also horizontal, also
    cheapest-at-top), just with a range instead of a single point.

    Complements cost_sensitivity_plot() (one model, the exact chart the
    report's Figure 6 caption names) rather than replacing it.
    """
    ...
    names = [n for n, _ in ranked][::-1]   # cheapest at top, barh convention
    data = [d for _, d in ranked][::-1]

    p_mid = [d["p"] for d in data]
    p_lo = [max(0.0, p - 0.10) for p in p_mid]   # downside: lower p
    p_hi = [min(1.0, p + 0.10) for p in p_mid]   # upside: higher p
    m_mid = [monthly_cost(d["l1"], p) for d, p in zip(data, p_mid)]
    m_downside = [monthly_cost(d["l1"], p) for d, p in zip(data, p_lo)]  # lower p -> higher cost
    m_upside = [monthly_cost(d["l1"], p) for d, p in zip(data, p_hi)]    # higher p -> lower cost

    fig, ax = plt.subplots(figsize=(10.8, 6.2))
    y = list(range(len(names)))

    # A restrained interval-and-dot treatment keeps the very wide log-scale
    # ranges legible without making them look like solid quantities.
    for yi, upside, downside in zip(y, m_upside, m_downside):
        ax.plot([upside, downside], [yi, yi], color=LIGHT_BLUE, linewidth=12,
                solid_capstyle="round", zorder=2)
    ax.scatter(m_upside, y, color=BLUE, edgecolor="white", linewidth=1,
               s=54, zorder=3, label="Upside (+10pp)")
    ax.scatter(m_downside, y, color=CORAL, edgecolor="white", linewidth=1,
               s=54, zorder=3, label="Downside (-10pp)")
    ax.scatter(m_mid, y, color=CORAL, edgecolor=NAVY, linewidth=1.4,
               s=125, zorder=4, label="Measured")
    ax.set_xscale("log")
    ax.set_yticks(y)
    ax.set_yticklabels(names, fontsize=11)
    ax.set_xlabel("Expected monthly cost (USD, log scale)  •  {:,} claims/month"
                  .format(VOLUME_PER_MONTH))
    ax.grid(axis="x", which="major", color="#D9DEE5", linewidth=0.8)
    ax.grid(axis="x", which="minor", color="#EEF1F4", linewidth=0.55)
    ax.set_axisbelow(True)
    for spine in ("top", "right", "left", "bottom"):
        ax.spines[spine].set_visible(False)
    ax.tick_params(axis="y", length=0, pad=12)
    ax.tick_params(axis="x", colors="#5C6470")

    for yi, plo, pm, phi, mlo, mm, mhi in zip(
            y, p_hi, p_mid, p_lo, m_upside, m_mid, m_downside):
        ax.annotate("${:,.0f}\n{:.0%}".format(mlo, plo), (mlo, yi),
                    xytext=(-9, 0), textcoords="offset points", ha="right",
                    va="center", fontsize=8, color=BLUE, linespacing=1.15)
        ax.annotate("${:,.0f}\n{:.0%}".format(mhi, phi), (mhi, yi),
                    xytext=(9, 0), textcoords="offset points", ha="left",
                    va="center", fontsize=8, color=CORAL, linespacing=1.15)
        ax.annotate("${:,.0f}  •  {:.0%}".format(mm, pm), (mm, yi),
                    xytext=(0, 15), textcoords="offset points", ha="center",
                    va="bottom", fontsize=8.5, color=NAVY, fontweight="bold")

    ax.set_xlim(min(m_upside) * 0.48, max(m_downside) * 2.1)
    ax.set_ylim(-0.55, len(names) - 0.18)
    ax.legend(ncol=3, frameon=False, loc="lower left", bbox_to_anchor=(0, 1.015),
              handletextpad=0.45, columnspacing=1.5, fontsize=9)
    fig.suptitle("Monthly cost sensitivity by model", fontsize=18,
                 fontweight="bold", color=NAVY, x=0.075, ha="left", y=0.985)
    fig.text(0.075, 0.925,
             "Range shows each model at ±10 percentage points from its measured success rate.",
             fontsize=10.5, color=GREY)
    fig.text(0.075, 0.035,
             "Key finding: even under downside assumptions, DeepSeek and Gemini remain cheaper than the other models' upside cases.",
             fontsize=9.2, color=NAVY)
    fig.tight_layout(rect=(0.06, 0.075, 0.985, 0.88))
    save(fig, "fig_d6_sensitivity_all_models.png")
...
def monthly_cost_by_model_plot():
    ...
    # Retain this figure's earlier navy/coral treatment at the user's request.
    colors = ["#273043"] * (len(names) - 1) + ["#E07A5F"]
...
def main():
    ...
    cost_sensitivity_all_models_plot()
