...
| LIU WEIQI | D6 three-layer cost model, four-lever ledger, sensitivity analysis, break-even calculation, and cap economics. | Landed and verified: `scripts/d6_cost_model.py` reproduces D6 from canonical live result files using the FAQ-defined trial pass rate. |
...
`scripts/make_fixtures_A.py`). Every member owns at least 5, grouped by the kind of
...
`scripts/make_fixtures_A.py`, regenerate, and write the label into
...
```bash
python3 scripts/make_fixtures_A.py
